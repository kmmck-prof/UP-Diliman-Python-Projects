...
def Get_Topology(resistor, graph, dictionary, destination_graph=None):
    ...
    ################################
    #then return if True
    allow, R_list, R_eq = Parallel_Pull(resistor, graph, dictionary)
    if(allow):
        #convert to integer
        R_eq = int(R_eq)
        
        #get the first resistor in the list,
        #then remove R via string indexing
        #then convert it into an integer
        #conclusion: integer can now be used in minheap touples to print in ascending order 
        R_first = int(R_list[0][1:])
        
        #################################################################################################
        # Realization: main function already processes resistors in ascending order. No need for touples
        #################################################################################################
        # Decision: keep it just in case it becomes needed in milestone 3
        #################################################################################################
